...
function RenderOutputExecuteResult({
  data,
  execution_count,
  metadata,
  output_type,
...
function RenderOutputs({ outputArray }) {
  ...
  return outputArray.map((output, idx) => {
    ...
    return OutputRenderComponent ? (
      <OutputRenderComponent key={idx} {...output} />
    ) : null;
  });
}
function RenderCode({ cell_type, execution_count, metadata, source, outputs }) {
  ...
  if (metadata.scrolled)
    return "Scrolled Cell Not Implemented"
  
  if (metadata.collapsed)
    return "Collapsed Cell Not Implemented"
...
export default function RenderNotebook({
  metadata,
  nbformat,
  nbformat_minor,
  cells,
