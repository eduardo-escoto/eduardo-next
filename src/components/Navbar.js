...
const PageNavigationListItem = ({ text, uri, customClasses }) => (
  <li className="nav-item">
    <Link href={uri}>
      <a aria-current="page" className={"nav-link" + customClasses}>
        {text}
      </a>
    </Link>
  </li>
      ...
      <div className="container-lg">
        <Link href="/">
          <div className="navbar-brand">
            ...
            />
            <a style={{ fontSize: "25px" }}>Eduardo Escoto</a>
          </div>
        </Link>
        <button
        ...
        <div className="collapse navbar-collapse" id="nav_dropdown">
          ...
          <ul className="navbar-nav navbar-right">
            {socialLinks.map((socialLink) => (
              <SocialsLinksListItem {...socialLink} key={socialLink.type} isPageWide={isPageWide}/>
            ))}
          </ul>
        </div>
      </div>
